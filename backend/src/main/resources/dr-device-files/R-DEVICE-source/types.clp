...
(deffunction discover-ref-type (?var $?condition)
	;(printout t "discover-ref-type: ?var: " ?var crlf)
	;(printout t "discover-ref-type: $?condition: " $?condition crlf)
	(if (= (length$ $?condition) 0)
	   then
	   	(create$)
	   else
		(bind ?p2 (get-token $?condition))
		;(printout t "?p2: " ?p2 crlf)
		(bind $?cond-elem (subseq$ $?condition 1 ?p2))
		;(bind $?rest-cond (subseq$ $?condition (+ ?p2 1) (length$ $?condition)))
	   	(if (or 
	   		(eq (nth$ 2 $?cond-elem) not)
	   	   ...
	   	   then
	   		   ...
	   		   then
	   		   	...
	   		   	(if (and (eq (nth$ 1 $?cond-elem) ?var)
	   		   		 (eq (nth$ 2 $?cond-elem) <-))
	   		   	   then
	   		   	   	(create$ INSTANCE-NAME (nth$ 4 $?cond-elem))
	   		   	   else
	   		   		(if (and (eq (nth$ 4 $?cond-elem) name)
