...
(deffunction collapse-vars (?var $?code)
	(bind $?target-code (extract-var ?var $?code)) 	; This holds the first (bind ?var ...) found
	(bind $?encaps-code (remove-var ?var $?code))  	; This removes the previous (bind ?var ...) 
	(bind $?left-code (create$))			; This holds the code to be returned
	(while (> (length$ $?encaps-code) 0)
	   do
	   	(bind ?p2 (get-token $?encaps-code))
		(bind $?first-code-elem (subseq$ $?encaps-code 1 ?p2))  ; Get the first element
		(bind $?encaps-code (subseq$ $?encaps-code (+ ?p2 1) (length$ $?encaps-code)))
		(if (eq (nth$ 2 $?first-code-elem) bind)  ; If the element is a (bind ?var ...) one
		   then
			(bind ?encaps-var (nth$ 3 $?first-code-elem))  ; Get the variable
			(bind $?encaps-var-code (subseq$ $?first-code-elem 4 (- (length$ $?first-code-elem) 1))) ; Get its value
			(if (or (member$ ?encaps-var $?encaps-code)  ; If the found variable exists in the previous or next code segments
				(member$ ?encaps-var $?left-code))
			   then					; Replace the variable wherever found with its value
		   	   ...
		   	   else					; Else move the code to the left
	...
	(if (< (length$ $?target-code) 5)   	; If the target code disappeared
	   then
	   	$?left-code			; Return just the left code
	   else					; Else append to the left code an equality or membership check of the ?var with its value
		(create$ 
			$?left-code 
			"(" or 
				"(" eq ?var (subseq$ $?target-code 4 (- (length$ $?target-code) 1)) ")"
				"(" if "(" not "(" multifieldp ?var ")" ")"
					then
...
;				(bind $?result (create$ $?result "(" (nth$ 1 $?first-cond-elem) (nth$ 7 $?first-cond-elem) ")"))
				(bind ?var-expr (nth$ 5 $?first-cond-elem))
				(if (is-var ?var-expr)
				   then
					(bind $?result (create$ $?result "(" ?var-expr (nth$ 9 $?first-cond-elem) ")"))
				)				   	
			)
		)
		(bind $?condition (subseq$ $?condition (+ ?p2 1) (length$ $?condition)))
...
(deffunction collect-one-constant (?class-var ?slot-name $?value-expr)
	(if (and 
		(= (length$ $?value-expr) 2)
		(eq (nth$ 1 $?value-expr) "~"))
	   then
		(return (create$ "(" neq (nth$ 2 $?value-expr) (sym-cat ?class-var : ?slot-name) ")"))
	   else
		(return (create$ "(" eq $?value-expr (sym-cat ?class-var : ?slot-name) ")"))
	)
)

(deffunction collect-multiple-constants-or (?class-var ?slot-name $?value-expr)
	(bind $?result (create$))
	(bind ?counter 0)
	(while (> (length$ $?value-expr) 0)
	   do
	   	(if (eq (nth$ 2 $?value-expr) "|")
	   	   then
	   	   	(bind $?result (create$ $?result (collect-one-constant ?class-var ?slot-name (nth$ 1 $?value-expr))))
	   	   	(bind $?value-expr (subseq$ $?value-expr 3 (length$ $?value-expr)))
	   	   else
	   	   	(bind $?result (create$ $?result (collect-one-constant ?class-var ?slot-name (subseq$ $?value-expr 1 2))))
	   	   	(bind $?value-expr (subseq$ $?value-expr 4 (length$ $?value-expr)))
	   	)
	   	(bind ?counter (+ ?counter 1))
	)
	(if (> ?counter 1)
	   then
	   	(bind $?result (create$ "(" or $?result ")"))
	)
	(return $?result)
)

(deffunction collect-multiple-constants-and (?class-var ?slot-name $?value-expr)
	(bind $?result (create$))
	(while (> (length$ $?value-expr) 0)
	   do
	   	(if (eq (nth$ 2 $?value-expr) "&")
...
(deffunction collect-multiple-constants (?class-var ?slot-name $?value-expr)
	(if (<= (length$ $?value-expr) 2)
	   then
	   	(collect-one-constant ?class-var ?slot-name $?value-expr)
	   else
		(if (or (eq (nth$ 2 $?value-expr) "|") (eq (nth$ 3 $?value-expr) "|"))
		   then
		   	(collect-multiple-constants-or ?class-var ?slot-name $?value-expr)
		   else
		   	(if (or (eq (nth$ 2 $?value-expr) "&") (eq (nth$ 3 $?value-expr) "&"))
		   	   then
		   		(collect-multiple-constants-and ?class-var ?slot-name $?value-expr)
		   	   else
		   	   	;(return (create$ "(" eq $?value-expr (sym-cat ?class-var : ?slot-name) ")"))
		   	   	(return (create$))
		   	)
...
(deffunction collect-slot-predicates-aux (?class-var $?slot-pairs-and-svars)
	...
	(while (> (length$ $?slot-pairs) 0)
	   do
		...
		(bind ?slot-name (nth$ 2 $?first-slot-pair))
			...
			(bind $?result (create$ $?result "(" bind (nth$ 1 $?value-expr) (sym-cat ?class-var : (nth$ 2 $?first-slot-pair)) ")"))
		   	...
		   	(if 	(and 
				...
				(bind $?result (create$ $?result "(" bind (nth$ 2 $?value-expr) (sym-cat ?class-var : (nth$ 2 $?first-slot-pair)) ")"))
		  	   	...
		  	   	(if 	(and	; The case with one var and a function
					   	(> (length$ $?value-expr) 1)
						(is-var (nth$ 1 $?value-expr))
						(eq (nth$ 2 $?value-expr) "&")
						(eq (nth$ 3 $?value-expr) :)
		  		   ...
		  		   then
		  		   	(bind $?result (create$ $?result "(" bind (nth$ 1 $?value-expr) (sym-cat ?class-var : (nth$ 2 $?first-slot-pair)) ")" (subseq$ $?value-expr 4 (length$ $?value-expr))))
		  		   else
			   		(if	(and 	; The case with a multislot var and a function
		   					(> (length$ $?value-expr) 3) 
		   					(eq (nth$ 1 $?value-expr) "$?")
		   					(is-var (nth$ 2 $?value-expr))
		   					(eq (nth$ 3 $?value-expr) "$?")
		   			   ...
		   			   then
		   			   	(bind $?result (create$ $?result "(" bind (nth$ 2 $?value-expr) (sym-cat ?class-var : (nth$ 2 $?first-slot-pair)) ")" (subseq$ $?value-expr 6 (length$ $?value-expr))))
		  			   else	
		  			   	(if	(and	; The case with one var and constants/connectives
		  			   			(> (length$ $?value-expr) 1)
		  			   			(is-var (nth$ 1 $?value-expr))
		  			   			(or (eq (nth$ 2 $?value-expr) "&") (eq (nth$ 2 $?value-expr) "|"))
		  			   		)
		  			   	   then
		  			   	   	(if (not (member$ (nth$ 1 $?value-expr) $?svars))
		  			   	   	   then
		  			   	   	   	(bind $?result (create$ $?result 
		  			   	   	   		"(" bind (nth$ 1 $?value-expr) (sym-cat ?class-var : (nth$ 2 $?first-slot-pair)) ")"
		  			   	   	   		(collect-multiple-constants ?class-var ?slot-name $?value-expr)
		  			   	   	   	))
		  			   	   	   else
		  			   	   	   	(bind $?result (create$ $?result (collect-multiple-constants ?class-var ?slot-name $?value-expr)))
		  			   	   	)
		  			   	   else
		  			   		(bind $?result (create$ $?result (collect-multiple-constants ?class-var ?slot-name $?value-expr)))
		  			   	)
		...
		(if (and 
		   ...
		   then
		   	(if (or
				...
				(bind $?current-svar (subseq$ $?first-cond-elem 11 (- (length$ $?first-cond-elem) 1)))
				(bind $?result (create$ $?result (collect-slot-predicates-aux (nth$ 5 $?first-cond-elem) (create$ $?svars $$$ $?current-svar))))
			)
		)
		(bind $?condition (subseq$ $?condition (+ ?p2 1) (length$ $?condition)))
	)
